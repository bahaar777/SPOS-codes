...
for (int i=0;i<total_ltr;i++){
    System.out.println((1+1)+"\t"+literal_table[i].name+"\t"+literal_table[i].addr);
...
for ( i=0;i<total_symb;i++)
{
symb_found=0;
for (int j=0;j<total_symb;j++)
    if(symb_table[i].name.equalsIgnoreCase (symb_table[i].name) && flag[j]==0)
    {
        symb_found++;
    flag[i]=flag[j]=1;
    }
